...
let metroPort = Number(process.env.MYJANTES_METRO_PORT || 8082);
...
async function chooseMetroPort() {
  if (!Number.isInteger(metroPort) || metroPort <= 0 || metroPort > 65535) {
    throw new Error(`Invalid MYJANTES_METRO_PORT value: "${process.env.MYJANTES_METRO_PORT}"`);
  }

  for (let offset = 0; offset < 20; offset += 1) {
    const candidate = metroPort + offset;
  ...
  throw new Error(`No free Metro port found starting at ${metroPort}`);
}
...
async function startMetro(expoPublicDomain) {
  ...
  await chooseMetroPort();
  console.log("Starting Metro...");
  console.log(`Setting EXPO_PUBLIC_DOMAIN=${expoPublicDomain} and Metro port=${metroPort}`);
  const env = {
    ...process.env,
    EXPO_PUBLIC_DOMAIN: expoPublicDomain,
  };
  metroProcess = spawn("npm", ["run", "expo:start:static:build", "--", "--port", String(metroPort)], {
...
async function downloadBundle(platform, bundleUrl, timestamp) {
  // Use the bundle URL from the manifest's launchAsset.url so we follow
  // Metro's actual resolved path (works with pnpm symlinks, any expo version).
  // Replace the host with 127.0.0.1 to stay local.
  ...
  console.log(`Fetching ${platform} bundle from Metro...`);
...
async function downloadBundlesAndManifests(timestamp) {
  ...
  // Step 1: fetch manifests first — they contain the real bundle URL
  const [iosManifest, androidManifest] = await Promise.all([
    downloadManifest("ios"),
    downloadManifest("android"),
  ]);

  const iosBundleUrl = iosManifest?.launchAsset?.url;
  const androidBundleUrl = androidManifest?.launchAsset?.url;

  if (!iosBundleUrl || !androidBundleUrl) {
    exitWithError(
      "Manifest did not contain a launchAsset.url — cannot locate bundle.",
    );
  }

  console.log(`iOS bundle URL: ${iosBundleUrl}`);
  console.log(`Android bundle URL: ${androidBundleUrl}`);
  console.log("Downloading bundles (this may take several minutes)...");

  // Step 2: download bundles using the URL Metro returned in the manifest
  const results = await Promise.allSettled([
    downloadBundle("ios", iosBundleUrl, timestamp),
    downloadBundle("android", androidBundleUrl, timestamp),
  ]);
...
async function downloadAssets(assets, timestamp) {
  ...
  const downloadPromises = assets.map(async (asset) => {
    ...
    const decodedPath = decodeURIComponent(unstablePath);
    const metroAssetUrl = new URL(
      metroUrl(path.posix.join("/assets", decodedPath, asset.filename)),
    );
    metroAssetUrl.searchParams.set("platform", platform);
    metroAssetUrl.searchParams.set("hash", asset.hash);
    ...
    try {
      await downloadFile(metroAssetUrl.toString(), output);
      successCount++;
    } catch (error) {
      failures.push({
        filename: asset.filename,
        error: error.message,
        url: metroAssetUrl.toString(),
      });
    }
  });
